...
def compile_shaders():
    vertex_shader_source = """
        ...
        void main(void)
        {
            float x_grid = float(gl_InstanceID % 10);
            float y_grid = float(gl_InstanceID / 10);
            vec4 instance_offset = vec4(x_grid - 4.5, y_grid - 4.5, 0.0, 0.0);
            gl_Position = P_matrix * V_matrix * ((M_matrix * position) + instance_offset);
